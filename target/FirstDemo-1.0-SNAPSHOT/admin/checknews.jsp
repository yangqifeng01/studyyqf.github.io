	...
	<body class="no-skin">
							...
							<div>
								...
								<div class="row">
									...
									<span class="col-md-6" style="margin: 5px 0">
										<label for="newsTitle">新闻标题：</label><input id="newsTitle" type="text" name="newsTitle" style="width: 400px;" value="${news.newsTitle}" readonly/>
									</span>
									<span class="col-md-6" style="margin: 5px 0">
										<label for="newsAuthor">新闻作者：</label><input id="newsAuthor" type="text" name="newsAuthor" style="width: 200px;" value="${news.newsAuthor}" readonly>
									</span>
									<span class="col-md-12" style="margin: 5px 0">
										<span><label style="text-align: center">新闻类型：</label></span>
										<label style="margin: 5px 0"><input type="radio" value="1" name="newsTypes" style="vertical-align: text-bottom;margin-bottom: 2px;" ${(1 eq news.newsTypes)?"checked":""} readonly> 旅游动态 </label>
										<label style="margin: 5px 0"><input type="radio" value="2" name="newsTypes" style="vertical-align: text-bottom;margin-bottom: 2px;" ${(2 eq news.newsTypes)?"checked":""} readonly> 旅游公告 </label>
									</span>
									<span class="col-md-12" style="margin: 5px 0">
										<label for="multipartFile">更换图片：</label>
										<img src="<%=basePath%>/${news.newsImage}" alt="${news.newsTitle}图片" style="width: 200px;height: 100px">
										<label for="multipartFile" style="margin: 5px 0">上传图片：</label ><input id="multipartFile" name="multipartFile" type="file" style="display: inline" readonly/>
									</span>
								</div>
								<label for="myEditor" style="margin: 5px 0">新闻内容：</label>
								<textarea id="myEditor" style="padding-top: 2px;padding-bottom:5px;width: 1030px;height: 200px" name="newsContent" readonly>${news.newsContent}</textarea>
								<%--超级管理员审核--%>
								<c:if test="${news.state == 0 && user.status == 3}">
		...
		<script>
			...
			function Check(state){
				if(state === 1){
					bootbox.prompt({
						...
						callback:function (result) {
							let data = {"newsId":${news.newsId},"state":state,"checkMsg":result};
							console.log(result);
							if (result){
								$.ajax({
									dataType: "json",
				...
				if(state === 2){
					let data = {"newsId":${news.newsId},"state":state,"checkMsg":null};
					$.ajax({
						dataType:"json",
						url:"${pageContext.request.contextPath}/admin/checkNews",
						data:data,
