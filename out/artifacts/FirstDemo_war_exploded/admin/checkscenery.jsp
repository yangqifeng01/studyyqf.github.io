	...
	<body class="no-skin">
			...
			<div class="main-content">
				...
				<div class="breadcrumbs" id="breadcrumbs">
					<ul class="breadcrumb">
						...
						<li>
							<a href="form-wizard.html">审核风景</a>
						</li>
					</ul><!-- /.breadcrumb -->
				...
				<div class="page-content">
					...
					<div class="page-content-area">
						<div class="row">
							<div class="col-xs-12">
							<div>
								<form action="#" class="form-inline" method="post" enctype="multipart/form-data" onsubmit="return false">
								<div class="row">
									<input id="id" name="id" value="${scenery.id}" style="display: none">
									<span class="col-md-6" style="margin: 5px 0">
										<label for="name">景点名称：</label><input id="name" type="text" name="name" style="width: 300px;" value="${scenery.name}" readonly/>
									</span>
									<span class="col-md-6" style="margin: 5px 0">
										<label for="openTime">开放时间：</label><input id="openTime" type="text" name="openTime" style="width: 400px;" value="${scenery.openTime}" readonly>
									</span>
									<span class="col-md-6" style="margin: 5px 0">
										<label for="address">景点位置：</label><input id="address" type="text"  name="address" style="width: 400px;" value="${scenery.address}" readonly/>
									</span>
									<span class="col-md-6" style="margin: 5px 0">
										<label for="rates">门票价格：</label><input id="rates" type="text" name="rates" value="${scenery.rates}" readonly>
									</span>
									<span class="col-md-12" style="margin: 5px 0">
										<label for=introduction>景点描述：</label><input id="introduction" type="text" name="introduction" style="width: 800px" value="${scenery.introduction}" readonly/>
									</span>
									<span class="col-md-12" style="margin: 5px 0">
										<label for="multipartFile">更换图片：</label>
										<img src="<%=basePath%>/${scenery.image}" alt="${scenery.name}图片" style="width: 200px;height: 100px" readonly>
								...
								<textarea id="myEditor" style="padding-top: 2px;padding-bottom:5px;width: 1030px;height: 200px" name="content" readonly>${scenery.content}</textarea>
									<%--超级管理员审核--%>
								<c:if test="${scenery.state == 0 && user.status == 3}">
		...
		<script>
			...
			function Check(state){
				if(state === 1){
					bootbox.prompt({
						...
						callback:function (result) {
							let data = {"id":${scenery.id},"state":state,"checkMsg":result};
							console.log(result);
							if (result){
								$.ajax({
									dataType: "json",
				...
				if(state === 2){
					let data = {"id":${scenery.id},"state":state,"checkMsg":null};
					$.ajax({
						dataType:"json",
						url:"${pageContext.request.contextPath}/admin/checkScenery",
						data:data,
