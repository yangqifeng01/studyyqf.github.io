					...
					<div class="page-content-area">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="row">
									<div class="col-md-6" >
										<a href="<%=basePath%>/admin/addfood.jsp" class="btn btn-default">添加美食</a>
										<a class="btn btn-default" href="<%=path%>/admin/">批量删除</a>
									</div>
									<div class="col-md-6">
										<div class="form-inline" style="float: right">
											<div class="form-group">
												<input type="text" class="form-control" placeholder="请输入美食名" id="nameString">
											</div>
											<div class="form-group">
										...
										<table id="sample-table-1" class="table table-striped table-bordered table-hover">
											<thead>
												<tr style="text-align: center">
													...
													<th>id</th><th  style="width: 80px">美食名称</th><th>美食位置</th><th>美食类型</th><th style="width: 80px">美食价格</th><th style="width: 80px">添加者</th><th>美食图片</th><th class="hidden-480" style="width: 80px">操作</th>
												</tr>
											</thead>
											<tbody>
												<c:forEach items="${allFoods}" var="food">
													<c:if test="${food.state == 3 || user.status == 3}">
														<tr>
															<td class="center"><label class="position-relative"><input type="checkbox" class="ace" /><span class="lbl"></span></label></td>
															<td><a href="#">${food.foodId}</a></td>
															<td>${food.foodName }</td>
															<td class="hidden-480"><span class="label label-sm label-warning">${food.foodAddress } </span></td>
															<c:choose><c:when test="${food.types==1}"><td>特色菜</td></c:when><c:when test="${food.types==2}"><td>风味小吃</td></c:when></c:choose>
															<td>${food.foodPrice}</td>
															<td>${food.user.userName }</td>
															<td><img src="<%=basePath%>/${food.foodImage}" alt="${food.foodName}图片" style="width: 200px;height: 100px"></td>
																<%--超级管理员用户对于管理员用户添加信息的审核--%>
															<c:if test="${food.state == 0 && user.status == 3}"><td><a class="btn btn-primary" href="${pageContext.request.contextPath}/admin/toCheckFood?foodId=${food.foodId}">审核</a></td></c:if>
															<c:if test="${food.state == 1 && user.status == 3}"><td><label class="label label-sm label-warning">修改中</label></td></c:if>
															<c:if test="${food.state == 2 && user.status == 3}"><td><label class="label label-sm label-warning">待发布中</label></td></c:if>
															<c:if test="${food.state == 3}">
																<td>
																	<div class="hidden-sm hidden-xs btn-group">
																		<div class="hidden-sm hidden-xs btn-group">
																			<a class="btn btn-xs btn-info" href="${pageContext.request.contextPath}/admin/toUpdateFood?foodId=${food.foodId}"><i class="ace-icon fa fa-pencil bigger-120"></i></a>&nbsp;&nbsp;
																			<button class="btn btn-xs btn-danger" onclick="Delete('${food.foodName}')"><i class="ace-icon fa fa-trash-o bigger-120"></i></button>
																		</div>
																	</div>
																</td>
															</c:if>
													...
													<c:if test="${food.authorId == user.userId && food.state != 3 && user.status == 2}">
														<tr>
															<td class="center"><label class="position-relative"><input type="checkbox" class="ace" /><span class="lbl"></span></label></td>
															<td><a href="#">${food.foodId}</a></td>
															<td>${food.foodName }</td>
															...
															<c:if test="${food.state == 1}">
																...
																<a class="btn btn-warning" style="margin-top: 5px" href="${pageContext.request.contextPath}/admin/toUpdateFood?foodId=${food.foodId}">修改</a></td>
															</c:if>
															<c:if test="${food.state == 2}">
																<td><button class="btn btn-success" onclick="issue(${food.foodId})">发布</button></td>
															</c:if>
														</tr>
													</c:if>
												</c:forEach>
										...
										<div class="row">
											...
											<div class="col-xs-11">
												<ul class="pagination">
													...
													<li><a href="${pageContext.request.contextPath}/admin/fenYe3?pageNum=1&pageSize=${pageInfo.pageSize}">首页</a></li>
													<li><a href="${pageContext.request.contextPath}/admin/fenYe3?pageNum=${pageInfo.pageNum-1}&pageSize=${pageInfo.pageSize}">上一页</a></li>
													<%--最多五个页面选项，大于5个则只显示五个--%>
													<c:forEach begin="1" end="${pageInfo.pages}" var="i">
														<li id="${i}"><a <c:if test="${pageInfo.pageNum == i}">style="background-color: #4e72b8"</c:if> href="${pageContext.request.contextPath}/admin/fenYe3?pageNum=${i}&pageSize=${pageInfo.pageSize}">${i}</a></li>
													</c:forEach>
													<li ><a href="${pageContext.request.contextPath}/admin/fenYe3?pageNum=${pageInfo.pageNum+1}&pageSize=${pageInfo.pageSize}">下一页</a></li>
													<li><a href="${pageContext.request.contextPath}/admin/fenYe3?pageNum=${pageInfo.pages}&pageSize=${pageInfo.pageSize}">尾页</a></li>
		...
		<script type="text/javascript">
			$(function(){
				if(window.history.replaceState){
					window.history.replaceState(null, null, "${pageContext.request.contextPath}/admin/foodlist.jsp")
				}
			})
			//指定当前页和页面数据数量
			function change(){
				...
				let pageNum = $("#pageNum");
				window.location.href= "${pageContext.request.contextPath}/admin/fenYe3?pageNum="+pageNum.val()+"&pageSize="+pageSize.val();
				...
				bootbox.confirm({
					...
					callback: function (result) {
						if (result) {
							$.ajax({
								"async":true,
								"url":"${pageContext.request.contextPath}/admin/deleteFood",
			...
			$("#search").click(function () {
				...
				if(name){
					$.ajax({
						url:"${pageContext.request.contextPath}/admin/selectFoodByFoodName",
						dataType: "json",
						data: {"foodName":name},
						type: "post",
						success:function (data){
							...
							if(!data){
								...
								bootbox.dialog({
									message:"<span class='bigger-110'> 美食id:"+data.foodId+",美食名:"+data.foodName+",位置:"+data.foodAddress+",美食价格:"+data.foodPrice+","
											+"美食类型:"+data.types+"</span>",
									buttons:
											{
												"success" :
														{
															...
															"callback": function() {
																window.location.href="${pageContext.request.contextPath}/admin/toUpdateFood?foodId="+data.foodId;
															}
														},
												"danger" :
														{
			...
			function issue(foodId) {
				let data = {"foodId":foodId,"state":3,"checkMsg":null};
				$.ajax({
					dataType:"json",
